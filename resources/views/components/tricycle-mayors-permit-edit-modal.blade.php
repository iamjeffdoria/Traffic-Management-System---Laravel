@props(['permit', 'tricycles'])

<div id="edit-tricycle-mayors-permit-modal-{{ $permit->id }}" class="hidden fixed inset-0 z-50 items-center justify-center px-4 py-6 overflow-y-auto">
    <div onclick="closeModal('edit-tricycle-mayors-permit-modal-{{ $permit->id }}')" class="absolute inset-0 bg-black/50"></div>
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div class="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
            ...
            <button type="button" onclick="closeModal('edit-tricycle-mayors-permit-modal-{{ $permit->id }}')" class="text-gray-400 hover:text-gray-600">
        ...
        <form id="tricycle-mayors-permit-edit-form-{{ $permit->id }}" data-permit-form method="POST" action="{{ route('tricycle.mayors-permit.update', $permit) }}" class="grid sm:grid-cols-2 gap-4 px-6 py-5 overflow-y-auto">
            @csrf
            @method('PUT')
                ...
                <select name="tricycle_id" required onchange="syncPermitTricycleFields(this)"
                    ...
                    @foreach ($tricycles as $tricycle)
                        <option value="{{ $tricycle->id }}"
                            ...
                            @selected($permit->tricycle_id === $tricycle->id)>
            ...
            <div>
                ...
                <input type="text" data-permit-name-display readonly value="{{ $permit->tricycle->name ?? '' }}"
                    class="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-600">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Address</label>
                <input type="text" data-permit-address-display readonly value="{{ $permit->tricycle->address ?? '' }}"
                    class="w-full rounded-lg border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm text-gray-600">
            </div>
        ...
        <div class="flex gap-3 px-6 py-4 border-t border-gray-100 shrink-0">
            <button type="submit" form="tricycle-mayors-permit-edit-form-{{ $permit->id }}"
                class="flex-1 rounded-full bg-red-600 text-white px-6 py-2.5 text-sm font-semibold hover:bg-red-700 transition-colors">
                Save Changes
            </button>
            <button type="button" onclick="closeModal('edit-tricycle-mayors-permit-modal-{{ $permit->id }}')"
