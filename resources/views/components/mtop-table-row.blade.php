...
<tr class="divide-x divide-gray-300 border-b border-gray-200 hover:bg-gray-50/60 transition-colors">
    <td class="px-4 py-4 align-top">
        <div class="flex items-center gap-1">
            ...
            <form id="delete-mtop-form-{{ $mtop->id }}" method="POST" action="{{ route('tricycle.mtop.destroy', $mtop) }}">
                @csrf
                @method('DELETE')
            </form>
            <button type="button" onclick="confirmMtopDelete('delete-mtop-form-{{ $mtop->id }}', 'MTOP {{ $mtop->case_no }}')" title="Delete"
