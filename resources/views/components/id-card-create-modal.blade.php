<div id="create-id-card-modal" class="hidden fixed inset-0 z-50 items-center justify-center px-4 py-6 overflow-y-auto">
    <div onclick="closeModal('create-id-card-modal')" class="absolute inset-0 bg-black/50"></div>
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        <div class="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
            <h3 class="font-semibold text-gray-900 text-lg">Add ID Card</h3>
            <button type="button" onclick="closeModal('create-id-card-modal')" class="text-gray-400 hover:text-gray-600">
        ...
        <form id="id-card-create-form" method="POST" action="{{ route('potpot.id-cards.store') }}" enctype="multipart/form-data" class="grid sm:grid-cols-2 gap-4 px-6 py-5 overflow-y-auto">
            @csrf
