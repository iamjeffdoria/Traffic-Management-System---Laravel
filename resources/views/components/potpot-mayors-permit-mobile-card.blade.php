...
<div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-4">
    <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
            ...
            <p class="text-gray-500 text-sm truncate">{{ $permit->name }} — {{ $permit->address }}</p>
        ...
        <div class="flex items-center gap-1 shrink-0">
            ...
            <form id="delete-potpot-permit-form-mobile-{{ $permit->id }}" method="POST" action="{{ route('potpot.mayors-permit.destroy', $permit) }}">
                @csrf
                @method('DELETE')
            </form>
            <button type="button" onclick="confirmPotpotPermitDelete('delete-potpot-permit-form-mobile-{{ $permit->id }}', 'Permit {{ $permit->control_no }}')" title="Delete"
